...
import { type SetCreate, type StagingSet as StagingSetRow, TABLES } from "./contract.ts"
    ...
    handlers: {
      ...
      listSets: () =>
        Effect.gen(function* () {
          ...
          return rows.filter((r) => !r.deleted).map(toState)
        }),
      ...
      chunk: ({ path, payload }: { path: { id: string }; payload: { text: string; startLine: number } }) =>
        Effect.gen(function* () {
          ...
          yield* batched.batch(applied.statements)
      ...
      deleteSet: ({ path }: { path: { id: string } }) =>
        Effect.gen(function* () {
          yield* requirePermission("staging:write")
          const set = yield* loadSet(store, path.id)
          yield* store.count(TABLES.sets) // first touch creates the table
