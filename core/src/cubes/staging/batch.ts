...
//
// HOW it stays safe: the kernel's Postgres store implements `batch` on every cube's store
// (core/src/pg/store.ts), and each batch runs inside one transaction under the cube's OWN
// role -- a statement aimed at another cube's schema dies in Postgres with a permission error,
// the same engine-enforced boundary every other operation sits behind. Staging composes SQL
// only for the two tables its manifest declares; field names travel as bound parameters
// (`body->'record'->>$1`), never as concatenated SQL.
//
 ...
 * a blind cast. The kernel builds the store from `storeFor`, which always includes `batch`; the
 * check keeps the cube honest if that ever stops being true.
 */
