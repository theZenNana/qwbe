...
export const fieldStats = (field: string, setId: string): SqlStatement => ({
  text: `SELECT count(v)::int AS filled, count(DISTINCT v)::int AS distinct_values,
           count(v) FILTER (WHERE v ~ $2)::int AS number_count,
           count(v) FILTER (WHERE v !~ $2 AND v ~ $3)::int AS date_count,
 ...
 * four specific shapes: free text, or an enum hiding in there. A field with few distinct
 * values overall is reported as `enum` -- the same value seen everywhere is a category, not
 * prose. Sensitive fields get `top` NEVER: the caller does not even fetch it, and passing no
 * top here is the second half of the suppression (tested in profile.test.ts).
 */
export const aggregateField = (
  ...
  if (other > 0) {
    shapes.push({ shape: stats.distinct_values <= ENUM_MAX_DISTINCT ? "enum" : "text", count: other })
  }
  ...
  if (top !== undefined) {
    profile.top = top.map((t) => ({ value: String(t.value), count: Number(t.count) }))
  }
  return profile
}
