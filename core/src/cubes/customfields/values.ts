// The values half of the customfields handlers.
//
...
export const valuesHandlers = (tools: PackTools) => ({
  ...
  valuesFor: ({ urlParams }: { urlParams: { readonly cube: string; readonly rowId: string } }) =>
    Effect.gen(function* () {
      // The lookup reads ANOTHER cube's rows, so it rides on THAT cube's own read permission
      // -- `customfields:read` alone would let a reader see custom values on rows the target
      // cube would refuse them.
  ...
  setValues: ({
    ...
    Effect.gen(function* () {
      ...
      // This endpoint RETURNS rowFields -- the target row's custom values -- so it rides on
      // the SAME gate the lookup rides on. A reader with `customfields:read` but without the
      // target cube's own read permission would otherwise read through here what `valuesFor`
      // refuses it.
      ...
      for (const [name, value] of Object.entries(values)) {
        ...
        // ONE validator for the whole system: the kernel's checkCustomValue, exported
        // through qwbe-core/custom-values.
  ...
  // The orphan report: values still in rows whose definition is gone. They are
  ...
  orphans: ({ urlParams }: { urlParams: { readonly cube: string } }) =>
    Effect.gen(function* () {
      ...
      // The report reads ANOTHER cube's rows, so it needs the
