...
import type { Catalogue, CubeDefinition, CubeTools } from "../../../../src/kernel/manifest.ts"
...
export const cube: CubeDefinition = {
  ...
  create: ({ store, bus, catalogue }: CubeTools) => {
    ...
    const mountedCubes = (): Catalogue => catalogue()
    ...
    return {
      ...
      handlers: {
        list: ({ urlParams }: { urlParams: typeof ListParams.Type }) =>
          Effect.gen(function* () {
        ...
        define: ({ payload }: { payload: typeof CustomFieldCreate.Type }) =>
          Effect.gen(function* () {
            ...
            const known = mountedCubes().map((c) => c.name)
        ...
        update: ({ path, payload }: { path: { id: string }; payload: typeof CustomFieldUpdate.Type }) =>
          Effect.gen(function* () {
            yield* requirePermission("customfields:write")
        ...
        valuesFor: ({ path }: { path: { cube: string; rowId: string } }) =>
          Effect.gen(function* () {
            yield* requirePermission("customfields:read")
        ...
        setValues: ({ path, payload }: { path: { cube: string; rowId: string }; payload: typeof ValuesWrite.Type }) =>
          Effect.gen(function* () {
